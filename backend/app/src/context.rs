...
use crate::dto::{AuthDto, BacklogDto, BoardDto, IssueDto, ProjectDto};
use crate::services::{
    BoardServiceImpl, IssueServiceImpl, ProjectServiceImpl, SearchServiceImpl,
};
use shared::{AppConfig, AppError, IssueId, ProjectKey};
...
impl AppContext {
    pub fn new(config: Arc<AppConfig>, repos: Arc<domain::Repositories>) -> Self {
        let auth: Arc<dyn AuthService> =
            Arc::new(JwtAuthService::new(config.auth.clone(), repos.users.clone()));
        let project: Arc<dyn ProjectService> =
            Arc::new(ProjectServiceImpl::new(repos.projects.clone()));
