...
> только по центральному `sub`, а все активные вошедшие пользователи имеют
> одинаковые пользовательские права. Ниже сохранены legacy/целевые разделы для
> исторического контекста; они не переопределяют этот runtime-контракт.
...
### 16.4. Интерфейс поиска

- Простой поиск применяет текстовый запрос после короткой паузы ввода; дополнительные фильтры на узких экранах сворачиваются.
- JQL выполняется по Enter или кнопке. Ошибки запроса и недоступность сервера показываются отдельно от пустой выдачи.
- Результаты показываются по 25 задач; номер страницы сохраняется в URL и сбрасывается при смене запроса, режима или фильтров.
