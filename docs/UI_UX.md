...
## 4b. Новые блоки Issue Detail (реализовано 2026-08)

### Вкладка «Вложения» (AttachmentPanel)

Четвёртая вкладка страницы задачи рядом с «Активность» / «Журнал работ». Функции:

- загрузка файла (кнопка + drag-зона не требуется), multipart `POST /issues/{id}/attachments`, лимит 25 МБ на файл;
- список вложений: имя, размер (человекочитаемый), дата, кнопки «Скачать» (blob-download через fetch с Bearer) и «Удалить»;
- ошибки загрузки (пустой файл, превышение лимита, чужая задача) показываются инлайн текстом destructive-цвета.

### Карточка «Метки» (LabelEditor)

В правой колонке Issue Detail под «Учёт времени». Функции:

- список прикреплённых меток (chip с цветом из `labels.color`);
- detach по клику на × в чипе;
- создание новой метки (имя + hex-цвет по умолчанию `#6b7280`) с немедленной привязкой;
- привязка существующих меток проекта через выпадающий список.

### Карточка «Связи» (LinkEditor)

Правая колонка Issue Detail, под «Метки». Функции:

- список связей с направлением: «блокирует DEMO-42» / «блокируется DEMO-7» (обратные связи показываются инверсной формулировкой, ссылка ведёт на целевую задачу);
- добавление связи: ввод ключа задачи (`DEMO-42`) + выбор типа (`блокирует` / `дублирует` / `связана с`);
- удаление связи кнопкой ×.

### Дропдаун перехода статуса

Селектор статуса в «Детали» показывает только разрешённые workflow-переходы из текущего статуса (из `GET /transitions`), а не все статусы. Запрещённые переходы отсекаются на бэкенде (400).

### Real-time

Все открытые страницы получают события `SSE /api/v1/events` (hook `useTrackerEvents` в AppShell): создание/изменение/перемещение задачи, комментарии — автоматически инвалидируют соответствующие TanStack Query (board, backlog, issue, comments). Вкладка браузера видит изменения другой вкладки без перезагрузки.
