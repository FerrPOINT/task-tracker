...
test('issue context and board remain usable on mobile and desktop', async ({ page, request }, testInfo) => {
  ...
  try {
    const project = await request.post(`${api}/projects`, {
      headers, data: { key, name, description: 'QA mobile layout' },
    })
    expect(project.ok(), await project.text()).toBeTruthy()
    created = true
    const issue = await request.post(`${api}/issues`, { headers, data: {
      project_key: key, issue_type: 'Task', summary: name, priority: 'Medium',
    } })
    ...
    await expect(page.locator(`a[href="/projects/${key}/backlog"]`).first()).toHaveAttribute('href', `/projects/${key}/backlog`)
    ...
    await expect(page.getByRole('tab', { name: /настройки инстанса|instance settings/i })).toBeVisible()
    ...
    await expect(page.getByRole('button', { name: /создать пользователя|create user/i })).toHaveCount(0)
  } finally {
    if (created) {
