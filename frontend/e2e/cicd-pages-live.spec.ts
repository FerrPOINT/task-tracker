...
const screenshotDir = fileURLToPath(
  new URL('../../../.local/screenshots/cicd-pages/', import.meta.url),
)
const base = 'http://localhost:7712'
const routes = [
  'dashboard',
  'projects',
  'repositories',
...
test('CI/CD routes remain usable across themes and viewports', async ({ page }) => {
      ...
      for (const [width, height] of viewports) {
        ...
        await expect
          .poll(
            ...
            { message: `CI/CD ${route} ${theme} ${width}px overflow` },
          )
        ...
        expect(nestedScrollers, `CI/CD ${route} ${theme} ${width}px nested scrollers`).toEqual([])
        ...
        expect(
          ...
          `CI/CD ${route} ${theme} ${width}px accessibility`,
        ).toEqual([])
      }
    }
  }
  expect(runtimeErrors, 'CI/CD console, page, network and server errors').toEqual([])
})
