...
const qaAccount = process.env.SDLC_LIVE_QA === '1'
  ? JSON.parse(readFileSync(fileURLToPath(new URL('../../../.local/qa-session.json', import.meta.url)), 'utf8')) as {
      email: string; password: string
    }
  : { email: '', password: '' }
const paths = [
  '/', '/services', '/services/admin-panel', '/revisions', '/branding',
  '/users', '/tokens', '/audit', '/runtime', '/settings',
]
const viewports = [[375, 812], [768, 1024], [1280, 800], [1920, 1080]] as const
  ...
  for (const path of paths) {
    ...
    for (const theme of ['dark', 'gray', 'light'] as const) {
      ...
      for (const [width, height] of viewports) {
        ...
        ), { message: `Admin ${path} ${theme} ${width}px overflow` }).toBeLessThanOrEqual(1)
        ...
        if ((theme === 'dark' && width === 375) || (theme === 'light' && width === 1280)) {
          ...
          expect(audit.violations.filter((issue) =>
            issue.impact === 'serious' || issue.impact === 'critical',
          ), `Admin ${path} ${theme} ${width}px`).toEqual([])
        }
      }
    }
  }
