...
type ActivityItem =
  | { type: 'comment'; id: string; data: Comment }
  | { type: 'worklog'; id: string; data: Worklog }
            ...
            <div key={item.id} className="flex gap-3 rounded-lg border border-border bg-surface p-4">
              ...
              <div className="min-w-0 flex-1">
                <div className="mb-1 flex flex-wrap items-center gap-2 text-sm">
                  <span className="font-medium text-text-primary">{c.authorName ?? t('comments.unknown')}</span>
                  <span className="text-text-muted">{t('issue.commented')}</span>
                  <span className="text-xs text-text-muted">{format(new Date(c.createdAt), 'yyyy-MM-dd HH:mm')}</span>
                </div>
          ...
          <div key={item.id} className="flex gap-3 rounded-lg border border-border bg-surface p-4">
            ...
            <div className="min-w-0 flex-1">
              <div className="mb-1 flex flex-wrap items-center gap-2 text-sm">
                ...
                <span className="text-text-muted">{t('issue.loggedTime', { duration: formatDuration(w.timeSpentSeconds) })}</span>
                <span className="text-xs text-text-muted">{format(new Date(w.createdAt), 'yyyy-MM-dd HH:mm')}</span>
              </div>
              {w.comment && (
